...
---Query 1---
-- Jugador con mayor numero de puntos por año y sus porcenatajes de tiros, considerando que fue titular en los partidos.
...
select distinct
	year,
	player_name,
	sum(fgm ) over (partition by year, player_name) as tiros_2_m, 
	sum(fga) over (partition by year, player_name) as tiros_2_a,
	sum(fg3m) over (partition by year, player_name)as tiros_3_m,
	sum(fg3a) over (partition by year, player_name) as tiros_3_a,
	sum(ftm) over (partition by year, player_name) as tiros_libres_m, 
	sum(fta) over (partition by year, player_name) as tiros_libres_a,	
	sum(pts) over (partition by year, player_name) as puntos
from 
	__info 
)
, __info3 as (
